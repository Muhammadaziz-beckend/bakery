...
class RequireOrganizationMixin:
    """Многие view фильтруют/сохраняют данные по request.user.organization,
    а это поле у User nullable — без этой проверки такой запрос падал бы
    IntegrityError'ом (500) на INSERT либо тихо возвращал пустой queryset."""

    def initial(self, request, *args, **kwargs):
        super().initial(request, *args, **kwargs)
        if request.user.is_authenticated and request.user.organization_id is None:
            raise ValidationError(
                {"detail": "У вас не указана организация. Обратитесь к администратору."}
            )
